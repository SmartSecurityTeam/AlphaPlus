...
local kicked_num = "تعداد اضا کیک شده: "..result.kicked_count.."\n"
...
local function lock_group_links(msg, data, target)
  ...
  if group_link_lock == 'yes' then
    return 'قفل لينک در حال حاضر فعال است'
  else
    data[tostring(target)]['settings']['lock_link'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'قفل لينک فعال شد'
  end
end
...
local function unlock_group_links(msg, data, target)
  ...
  if group_link_lock == 'no' then
    return 'قفل لينک در حال حاضر غير فعال است'
  else
    data[tostring(target)]['settings']['lock_link'] = 'no'
    save_data(_config.moderation.data, data)
    return 'قفل لينک غير فعال شد'
  end
end
...
local function lock_group_spam(msg, data, target)
  ...
  if not is_owner(msg) then
    return "فقط خريدار گروه!"
  end
  local group_spam_lock = data[tostring(target)]['settings']['lock_spam']
  if group_spam_lock == 'yes' then
    return 'قفل اسپم در حال حاضر فعال است'
  else
...
local function unlock_group_spam(msg, data, target)
  ...
  if group_spam_lock == 'no' then
    return 'قفل اسپم در حال حاضر غير فعال است'
  else
    data[tostring(target)]['settings']['lock_spam'] = 'no'
    save_data(_config.moderation.data, data)
    return 'قفل اسپم غير فعال شد'
  end
end
...
local function lock_group_flood(msg, data, target)
  ...
  if group_flood_lock == 'yes' then
    return 'قفل فلود در حال حاضر غير فعال است'
  else
...
local function lock_group_arabic(msg, data, target)
  ...
  if group_arabic_lock == 'yes' then
    return 'قفل عربي در حال حاضر فعال است'
  else
    data[tostring(target)]['settings']['lock_arabic'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'قفل عربي فعال شد'
  end
end
...
local function unlock_group_arabic(msg, data, target)
  ...
  if group_arabic_lock == 'no' then
    return 'قفل عربي در حال حاضر غير فعال است'
  else
...
local function lock_group_number(msg, data, target)
  ...
  if group_number_lock == 'yes' then
    return 'قفل عدد در حال حاضر فعال است'
  else
...
local function unlock_group_number(msg, data, target)
  ...
  if group_number_lock == 'no' then
    return 'قفل عدد در حال حاضر غير فعال است'
  else
    data[tostring(target)]['settings']['lock_number'] = 'no'
    save_data(_config.moderation.data, data)
    return 'قفل عدد غير فعال شد'
  end
end
...
local function lock_group_membermod(msg, data, target)
  ...
  if group_member_lock == 'yes' then
    return 'قفل اعضا در حال حاضر فعال است'
  else
...
local function unlock_group_membermod(msg, data, target)
  ...
  if group_member_lock == 'no' then
    return 'قفل اعضا در حال حاضر غير فعال است'
  else
    data[tostring(target)]['settings']['lock_member'] = 'no'
    save_data(_config.moderation.data, data)
    return 'قفل اعضا غير فعال شد'
  end
end
...
local function lock_group_rtl(msg, data, target)
  ...
  if group_rtl_lock == 'yes' then
    return 'قفل ار تي ال در حال حاضر فعال است'
  else
    data[tostring(target)]['settings']['lock_rtl'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'قفل ار تي ال فعال شد'
  end
end
...
local function unlock_group_rtl(msg, data, target)
  ...
  if group_rtl_lock == 'no' then
    return 'قفل ار تي ال در حال حاضر غير فعال است'
  else
    data[tostring(target)]['settings']['lock_rtl'] = 'no'
    save_data(_config.moderation.data, data)
    return 'قفل ار تي ال غير فعال شد'
  end
end
...
local function lock_group_tag(msg, data, target)
  ...
  if group_tag_lock == 'yes' then
    return 'قفل تگ در حال حاضر فعال است'
  else
...
local function unlock_group_tag(msg, data, target)
  ...
  if group_tag_lock == 'no' then
    return 'قفل تگ در حال حاضر غير فعال است'
  else
    data[tostring(target)]['settings']['lock_tag'] = 'no'
    save_data(_config.moderation.data, data)
    return 'قفل تگ غير فعال شد'
  end
end
...
local function lock_group_badw(msg, data, target)
  ...
  if group_badw_lock == 'yes' then
    return 'قفل فحش در حال حاضر فعال است'
  else
...
local function unlock_group_badw(msg, data, target)
  ...
  if group_badw_lock == 'no' then
    return 'قفل فحش در حال حاضر غير فعال است'
  else
    data[tostring(target)]['settings']['lock_badw'] = 'no'
    save_data(_config.moderation.data, data)
    return 'قفل فحش غير فعال شد'
  end
end
...
local function lock_group_cmd(msg, data, target)
  ...
  if group_cmd_lock == 'yes' then
    return 'قفل دستورات در حال حاضر فعال است'
  else
...
local function unlock_group_cmd(msg, data, target)
  ...
  if group_cmd_lock == 'no' then
    return 'قفل دستورات در حال حاضر غير فعال است'
  else
    data[tostring(target)]['settings']['lock_cmd'] = 'no'
    save_data(_config.moderation.data, data)
    return 'قفل دستورات غير فعال شد'
  end
end
...
local function lock_group_tgservice(msg, data, target)
  ...
  if group_tgservice_lock == 'yes' then
    return 'قفل تلگرام سرويس در حال حاضر فعال است'
  else
    data[tostring(target)]['settings']['lock_tgservice'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'قفل تلگرام سرويس فعال شد'
  end
end
...
local function unlock_group_tgservice(msg, data, target)
  ...
  if group_tgservice_lock == 'no' then
    return 'قفل تلگرام سرويس در حال حاضر غير فعال است'
  else
    data[tostring(target)]['settings']['lock_tgservice'] = 'no'
    save_data(_config.moderation.data, data)
    return 'قفل تلگرام سرويس غير فعال شد'
  end
end

local function lock_group_inlinekey(msg, data, target)
  if not is_momod(msg) then
    return
  end
  local group_inlinekey_lock = data[tostring(target)]['settings']['lock_inlinekey']
  if group_inlinekey_lock == 'yes' then
    return 'قفل دکمه شیشه ای در حال حاضر فعال است'
  else
    data[tostring(target)]['settings']['lock_inlinekey'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'قفل دکمه شیشه ای فعال شد'
  end
end

local function unlock_group_inlinekey(msg, data, target)
  if not is_momod(msg) then
    return
  end
  local group_inlinekey_lock = data[tostring(target)]['settings']['lock_inlinekey']
  if group_inlinekey_lock == 'no' then
    return 'قفل دکمه شیشه ای در حال حاضر غير فعال است'
  else
    data[tostring(target)]['settings']['lock_inlinekey'] = 'no'
    save_data(_config.moderation.data, data)
    return 'قفل دکمه شیشه ای غير فعال شد'
  end
end
...
local function lock_group_sticker(msg, data, target)
  ...
  if group_sticker_lock == 'yes' then
    return 'قفل استيکر در حال حاضر فعال است'
  else
    data[tostring(target)]['settings']['lock_sticker'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'قفل استيکر فعال شد'
  end
end
...
local function unlock_group_sticker(msg, data, target)
  ...
  if group_sticker_lock == 'no' then
    return 'قفل استيکر در حال حاضر غير فعال است'
  else
    data[tostring(target)]['settings']['lock_sticker'] = 'no'
    save_data(_config.moderation.data, data)
    return 'قفل استيکر غير فعال شد'
  end
end
...
local function lock_group_fwd(msg, data, target)
  ...
  if group_fwd_lock == 'yes' then
    return 'قفل فوروارد در حال حاضر فعال است'
  else
...
local function unlock_group_fwd(msg, data, target)
  ...
  if group_fwd_lock == 'no' then
    return 'قفل فوروارد در حال حاضر غير فعال است'
  else
    data[tostring(target)]['settings']['lock_fwd'] = 'no'
    save_data(_config.moderation.data, data)
    return 'قفل فوروارد غير فعال شد'
  end
end
...
local function lock_group_operator(msg, data, target)
  ...
  if group_operator_lock == 'yes' then
    return 'قفل اپراتور در حال حاضر فعال است'
  else
...
local function unlock_group_operator(msg, data, target)
  ...
  if group_operator_lock == 'no' then
    return 'قفل اپراتور در حال حاضر غير فعال است'
  else
    data[tostring(target)]['settings']['operator'] = 'no'
    save_data(_config.moderation.data, data)
    return 'قفل اپراتور غير فعال شد'
  end
end
...
local function lock_group_contacts(msg, data, target)
  ...
  if group_contacts_lock == 'yes' then
    return 'قفل مخاطب در حال حاضر فعال است'
  else
...
local function unlock_group_contacts(msg, data, target)
  ...
  if group_contacts_lock == 'no' then
    return 'قفل مخاطب در حال حاضر غير فعال است'
  else
    data[tostring(target)]['settings']['lock_contacts'] = 'no'
    save_data(_config.moderation.data, data)
    return 'قفل مخاطب غير فعال شد'
  end
end
...
local function lock_group_hashtag(msg, data, target)
  ...
  if group_hashtag_lock == 'yes' then
    return 'قفل هشتگ در حال حاضر فعال است'
  else
...
local function unlock_group_hashtag(msg, data, target)
  ...
  if group_hashtag_lock == 'no' then
    return 'قفل هشتگ در حال حاضر غير فعال است'
  else
    data[tostring(target)]['settings']['lock_hashtag'] = 'no'
    save_data(_config.moderation.data, data)
    return 'قفل هشتگ غير فعال شد'
  end
end
...
if data[tostring(target)]['settings'] then
		if not data[tostring(target)]['settings']['lock_inlinekey'] then
			data[tostring(target)]['settings']['lock_inlinekey'] = 'no'
		end
end
      if data[tostring(target)]['settings'] then
...
function get_message_callback(extra, success, result)
	...
	elseif get_cmd == "setowner" then
		...
		local channel_id = 'channel#id'..result.to.peer_id
			...
			if result.from.username then
				text = "@"..result.from.username.." [ "..result.from.peer_id.." ] خریدار گروه شد"
			else
				text = "[ "..result.from.peer_id.." ] خریدار گروه شد"
			end
			send_large_msg(channel_id, text)
		end
	...
	elseif get_cmd == 'mute_user' then
		...
		if is_muted_user(chat_id, user_id) then
			...
			send_large_msg(receiver, "["..user_id.."] از سایلنت لیست حذف شد :)")
		elseif is_admin1(msg) then
			mute_user(chat_id, user_id)
			send_large_msg(receiver, " ["..user_id.."] به سایلنت لیست اضافه شد :)")
		end
	end
end
...
elseif get_cmd == "setadmin" then
   for k,v in pairs(result) do
    ...
    if vusername == member or vpeer_id == memberid then
      ...
      channel_set_admin(channel_id, user_id, ok_cb, false)
      if v.username then
        text = "@"..v.username.." ["..v.peer_id.."] به عنوان ادمین تنظیم شد :)"
        savelog(msg.to.id, name_log.." ["..msg.from.id.."] set admin @"..v.username.." ["..v.peer_id.."]")
      else
        text = "["..v.peer_id.."] به عنوان ادمین تنظیم شد :)"
	...
	for k,v in pairs(result) do
		...
		if vusername == member or vpeer_id == memberid then
			...
			if group_owner then
				...
				if result.username then
					text = member_username.." ["..v.peer_id.."] خریدار شد"
				else
					text = "["..v.peer_id.."] خریدار شد"
				end
			end
...
local function run(msg, matches)
	...
	local data = load_data(_config.moderation.data)
		...
		if matches[1] == "باتها" and is_momod(msg) then
			member_type = 'Bots'
		...
		if matches[1] == 'ساخت لینک' and is_momod(msg)then
			...
			local receiver = get_receiver(msg)
				if success == 0 then
					send_large_msg(receiver, 'ارور بات سازنده گپ نیست ! :)')
		...
		if matches[1] == 'لینک' then
			...
			if not group_link then
				return "اول از همه با دستور (ساخت لینک) برای خودتون لینک بسازید !"
			end
		...
		if matches[1] == 'برکناری ادمین' then
			if not is_support(msg.from.id) and not is_owner(msg) then
				return
			end
			if type(msg.reply_id) ~= "nil" then
			...
			elseif matches[1] == 'برکناری ادمین' and matches[2] and string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local user_id = "user#id"..matches[2]
				local get_cmd = 'demoteadmin'
				user_info(user_id, cb_user_info, {receiver = receiver, get_cmd = get_cmd})
			elseif matches[1] == 'برکناری ادمین' and matches[2] and not string.match(matches[2], '^%d+$') then
				local cbres_extra = {
					channel = get_receiver(msg),
					get_cmd = 'demoteadmin'
				}
		...
		if matches[1] == 'تنظیم خریدار' and is_owner(msg) then
			if type(msg.reply_id) ~= "nil" then
				local cbreply_extra = {
					get_cmd = 'setowner',
					msg = msg
			...
			elseif matches[1] == 'تنظیم خریدار' and matches[2] and string.match(matches[2], '^%d+$') then
		--[[	local group_owner = data[tostring(msg.to.id)]['set_owner']
				if group_owner then
					local receiver = get_receiver(msg)
				...
				local user_id = matches[2]
				channel_get_users (receiver, in_channel_cb, {get_cmd=get_cmd, receiver=receiver, msg=msg, user_id=user_id})
			elseif matches[1] == 'تنظیم خریدار' and matches[2] and not string.match(matches[2], '^%d+$') then
				local	get_cmd = 'setowner'
				local	msg = msg
				local username = matches[2]
				local username = string.gsub(matches[2], '@', '')
		...
		if matches[1] == 'ممبر' then
			...
			elseif matches[1] == 'ممبر' and matches[2] and string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local user_id = "user#id"..matches[2]
				local get_cmd = 'demote'
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] demoted user#id"..matches[2])
				user_info(user_id, cb_user_info, {receiver = receiver, get_cmd = get_cmd})
			elseif matches[1] == 'ممبر' and matches[2] and not string.match(matches[2], '^%d+$') then
				local cbres_extra = {
					channel = get_receiver(msg),
					get_cmd = 'demote'
				}
		...
		if matches[1] == 'قفل' and is_momod(msg) then
			...
			if matches[2] == 'دکمه شیشه ای' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] locked inlinekey")
				return lock_group_inlinekey(msg, data, target)
			end
		...
		if matches[1] == 'بازکردن' and is_momod(msg) then
			...
			if matches[2] == 'دکمه شیشه ای' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] unlocked inlinekey")
				return unlock_group_inlinekey(msg, data, target)
			end
			if matches[2] == 'اپراتور' then
		...
		if matches[1] == 'تنظیم حساسیت' then
			...
			return 'حساسیت اسپم تغییر کرد به: '..matches[2]
		end
		...
		if matches[1] == 'آزادکردن' and is_momod(msg) then
			local chat_id = msg.to.id
			if matches[2] == 'موسیقی' then
			local msg_type = 'Audio'
		...
		if matches[1] == '0000' and not is_owner(msg) then
